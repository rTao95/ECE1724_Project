...
// Modules from your existing code
mod chunker;
mod file_operations;
mod storage_manager;
mod utils;
...
// NEW: A structure to track ongoing downloads
...
// We'll store a map from filename -> DownloadState
...
// NEW: A message requesting a file by name.
...
// NEW: A message announcing file metadata (how many chunks).
#[derive(serde::Serialize, serde::Deserialize)]
...
pub async fn run_peer_to_peer_system(
    ...
    // NEW: Keep track of ongoing downloads
...
async fn handle_user_input(
    ...
    // NEW: Handle download request
        ...
        if parts.len() < 2 {
            ...
            // Create and send FileRequestMessage
                ...
                .publish(topic.clone(), serialized)
            {
                ...
                // Initialize a download state (we will fill in total_chunks when we get metadata)
...
async fn handle_received_message(
    ...
    // Try deserializing into different message types:
    if let Ok(file_message) = serde_json::from_slice::<FileMessage>(data) {
        ...
        if file_message.password == password {
            ...
            // If this is part of a download we requested, try to parse chunk info
            // Filenames are in format: original_file_name(i-of-n)
            if let Some((original_name, i, n)) = parse_chunk_filename(&file_message.file_name) {
                ...
                if let Some(download_state) = dls.get_mut(&original_name) {
                    if download_state.total_chunks == 0 {
                        // We should have received metadata first, but if not, assume n
                    ...
                    download_state
                        .chunks_data
                        .insert(i, file_message.data.clone());
                    download_state.received_chunks += 1;
                    println!(
                        "Download progress for '{}': {}/{}",
                        original_name, download_state.received_chunks, download_state.total_chunks
                    );

                    // If all chunks received
                    if download_state.received_chunks == download_state.total_chunks {
                        // Reassemble file
                        let output_path = format!("{}/{}", local_peer_id_str, original_name);
                        let mut output_file = fs::File::create(&output_path)?;
                        for chunk_idx in 1..=download_state.total_chunks {
                            if let Some(chunk_data) = download_state.chunks_data.get(&chunk_idx) {
                                output_file.write_all(chunk_data)?;
                            } else {
                                println!("Missing chunk {} for '{}'", chunk_idx, original_name);
                                break;
                            }
                        }
                        println!("Download complete: {}", output_path);

                        // We can remove the download state now
                        dls.remove(&original_name);
                    }
                }
            }

            // Update logs and score as before
        ...
        // NEW: File Request Message
        if request_msg.password == password {
            // Check if we have the requested file in our local directory
            let local_peer_id_str = format!("./{}/", local_peer_id);
            let maybe_file_chunks =
                find_chunks_for_file(&local_peer_id_str, &request_msg.file_name);

            if let Some((total_chunks, chunk_files)) = maybe_file_chunks {
                // Send FileMetadataMessage first
                ...
                // Then send all chunks as FileMessage
        ...
        if meta_msg.password == password {
            // Update the download state with the total_chunks info
            let mut dls = downloads.lock().unwrap();
...
// NEW: Parse a chunk filename of the form "file_a.txt(1-of-5)"
fn parse_chunk_filename(name: &str) -> Option<(String, usize, usize)> {
    // Find the last '('
    let idx = name.rfind('(')?;
    // substring after '(' is "1-of-5)"
    let meta = &name[idx + 1..name.len() - 1]; // remove trailing ')', now "1-of-5"
    ...
    // original_name is everything before '('
    let original_name = (&name[..idx]).to_string();
    Some((original_name, i, n))
}

// NEW: Find all chunks for a given file (e.g. "file_a.txt") in a directory
fn find_chunks_for_file(dir: &str, file_name: &str) -> Option<(usize, Vec<String>)> {
    ...
    if chunk_files.is_empty() {
        ...
        // Sort chunk_files by their chunk index to send in order
